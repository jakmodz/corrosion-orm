...
use crate::{
    driver::{
        ...
        error::DriverError,
        sqlite_driver::{
            sqlite_config::SqliteConfig, sqlite_connection::CorrosionSqliteConnection,
        },
...
pub struct CorrosionSqlitePool {
    inner: sqlx::pool::Pool<sqlx::Sqlite>,
}

impl ConnectionPool for CorrosionSqlitePool {
    type Conn = CorrosionSqliteConnection;
    ...
    async fn new_pool(config: Self::Config) -> Result<Self, CorrosionOrmError> {
        ...
        Ok(CorrosionSqlitePool { inner: pool })
    }
    ...
    async fn acquire_conn(&self) -> Result<ConnectionGuard<Self>, CorrosionOrmError> {
        ...
        Ok(ConnectionGuard::new(CorrosionSqliteConnection {
            inner: conn,
        }))
    }
    ...
    async fn begin_transaction(&self) -> Result<Transaction<Self>, CorrosionOrmError> {
        let conn = self.inner.acquire().await.map_err(DriverError::Sqlx)?;
        let mut wrapped_conn = CorrosionSqliteConnection { inner: conn };
