use corrosion_orm::Model;
use corrosion_orm::prelude::*;

#[derive(Model, Debug)]
#[Table(name = "users")]
pub struct User {
    #[PrimaryKey]
    pub id: i64,
    pub name: String,
}

#[tokio::main]
async fn main() -> Result<(), CorrosionOrmError> {
    // 1. Connect to an in-memory SQLite database
    let db = corrosion_orm::connect(":memory:").await?;
    let mut conn = db.acquire_conn().await?;

    // 2. Create the "users" table from the model's schema
    {
        let mut ctx = QueryContext::from_model(User::get_schema(), conn.get_dialect());
        conn.execute_query(&mut ctx).await?;
    }

    // 3. Insert some sample data
    for i in 1..=100 {
        let user = User {
            id: i as i64,
            name: format!("User{}", i),
        };
        user.save(&mut conn).await?;
    }
    // 4. Get paginator and set the size of page
    let mut paginator = User::find().paginate(2);
    // 5. Fetch pages and print them
    if let Some(page) = paginator.fetch_next(&mut conn).await? {
        println!("Page 1: {:?}", page);
        if let Some(page) = paginator.fetch_next(&mut conn).await? {
            println!("Page 2: {:?}", page);
        }
    }
    // 6. Fetch remaining pages and print them
    while let Some(page) = paginator.fetch_next(&mut conn).await? {
        println!("Page: {:?}", page);
    }
    Ok(())
}
